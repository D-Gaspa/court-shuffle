...
import { STRAIGHT_SET_SCORE, TIEBREAK_SET_SCORE } from "../../support/constants.mjs"
...
test("imports history with arbitrary completed tournament scores", () => {
    ...
    state.history[0].rounds[0].scores[0].sets = TIEBREAK_SET_SCORE

    const imported = parseStateImport(JSON.stringify(state))

    assert.deepEqual(imported.state.history[0].rounds[0].scores[0].sets, TIEBREAK_SET_SCORE)
})
...
test("parse rejects out-of-bounds tournament series indexes", () => {
    const state = cloneSampleState()
    state.activeSession = createTournamentSeriesFixture(INVALID_TOURNAMENT_INDEX)
