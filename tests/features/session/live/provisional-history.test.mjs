...
import { STRAIGHT_SET_SCORE, STRAIGHT_SET_SCORE_ALT } from "../../../support/constants.mjs"
...
function createPlayedTournamentSession() {
    ...
    session.rounds[0].scores = [{ court: 1, sets: STRAIGHT_SET_SCORE }]
...
test("syncProvisionalHistory keeps one evolving live history entry and finalizes it in place", () => {
    ...
    session.rounds[0].scores[0] = { court: 1, sets: STRAIGHT_SET_SCORE_ALT }
    syncProvisionalHistory(state)
    assert.equal(state.history.length, 1)
    assert.deepEqual(state.history[0].tournamentSeries.tournaments[0].rounds[0].scores[0].sets, STRAIGHT_SET_SCORE_ALT)
