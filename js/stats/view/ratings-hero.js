...
function buildRatingsHeroCopy({ activeSeason, isArchivedView }) {
    if (!activeSeason) {
...
function createRatingsHero({
    ...
    intro.appendChild(createEl("p", "stats-hero-copy", buildRatingsHeroCopy({ activeSeason, isArchivedView })))
...
function createRatingsHeroControls({
    ...
    controls.appendChild(createModeToggle(selectedMode, onSelectMode))
